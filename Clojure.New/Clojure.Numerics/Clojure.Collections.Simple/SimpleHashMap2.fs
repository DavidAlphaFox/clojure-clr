...
and SHMNode2 =
    ...
    member this.getNodeSeq() =
        ...
        | BitmapNode2 (Bitmap = bitmap; Entries = entries) -> BitmapNode2Seq.create (entries, 0)
        | CollisionNode2 (Hash = hash; Count = count; KVs = kvs) -> CollisionNode2Seq.create (kvs, 0)
    ...
    static member pack (count: int) (nodes: SHMNode2 option []) (idx: int) : SHMNode2 =
        let newArray: BNode2Entry [] = count - 1 |> Array.zeroCreate
        let mutable j = 0
...
and BitmapNode2Seq(entries: BNode2Entry [], idx: int, seq: ISeq) =
    inherit ASeq()

    static member create(entries: BNode2Entry [], idx: int) : ISeq =
