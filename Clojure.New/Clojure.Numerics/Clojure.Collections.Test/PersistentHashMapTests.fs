...
let basicPersistentHashMapCreateTests =
    testList "Basic PersistentHashMap create tests" [
...
let basicPersistentHashMapAssocTests =
    testList "Basic P.H.M Assoc tests" [
...
let basicPersistentHashMapPersistentCollectionTests =
    testList "Basic P.H.M PersistentCollection tests" [
...
let basicPersistentHashMapPersistentMapTests =
    testList "Basic P.H.M PersistentMap tests" [
...
let aPersistentMapTests =
    testList "APersistentMap tests for PersistentHashMap" [
        ...
        testCase "Associative.assoc works" <| fun _ ->
            ...
            let a = PersistentHashMap.create(d) :> Associative
            let a1 = a.assoc(3,"c")
            let a2 = a.assoc(2,"c")
        ...
        testCase "Dictionary operations fail as necessary" <| fun _ ->
            ...
            let id = PersistentHashMap.create(d) :?> IDictionary
        ...
        testCase "ICollection goodies work" <| fun _ ->
            ...
            let c = PersistentHashMap.create(d) :?> ICollection
...
let PersistentHashMapIObjTests = 
    testList "PersistentHashMap IObj tests" [
        
        testCase "Verify PersistentHashMap.IObj" <| fun _ ->
            let d : Dictionary<int,string> = Dictionary()
            d.[1] <- "a"
            d.[2] <- "b"
...
[<Tests>]
let collisionTests = 
    testList "PersistentHashMap collision tests" [
        testCase "Collisions 100 10" <| fun _ ->
